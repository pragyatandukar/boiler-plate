...
class Application(Generic[_AppResult]):
    ...
    def __init__(
        ...
        color_depth: (ColorDepth | Callable[[], ColorDepth | None]
                      | None) = None,
    ...
    async def run_async(
        ...
        async def _run_async(f: asyncio.Future[_AppResult]) -> _AppResult:
            ...
            with self.input.raw_mode(), self.input.attach(
                    read_from_input_in_context), attach_winch_signal_handler(
                        self._on_resize):
                # Draw UI.
                self._request_absolute_cursor_position()
                self._redraw()
            ...
            if handle_sigint:
                with _restore_sigint_from_ctypes():
                    ...
                    loop.add_signal_handler(
                        signal.SIGINT,
                        lambda *_: loop.call_soon_threadsafe(self.key_processor
                                                             .send_sigint),
                    )
                    try:
                        yield
        ...
        def set_exception_handler_ctx(
                loop: AbstractEventLoop) -> Iterator[None]:
        ...
        @contextmanager
        def create_future(
            loop: AbstractEventLoop, ) -> Iterator[asyncio.Future[_AppResult]]:
    ...
    def run(
        ...
        def _called_from_ipython() -> bool:
            try:
                return (sys.modules["IPython"].version_info < (8, 18, 0, "")
    ...
    def _breakpointhook(self, *a: object, **kw: object) -> None:
        ...
        class CustomPdb(pdb.Pdb):
            ...
            def trace_dispatch(self, frame: FrameType, event: str,
                               arg: Any) -> TraceDispatch:
    ...
    def create_background_task(
            self, coroutine: Coroutine[Any, Any, None]) -> asyncio.Task[None]:
        """
    ...
    def _on_background_task_done(self, task: asyncio.Task[None]) -> None:
        ...
        if exc is not None:
            get_running_loop().call_exception_handler({
                "message":
                f"prompt_toolkit.Application background task {task!r} "
    ...
    def print_text(self,
                   ...
                   style: BaseStyle | None = None) -> None:
        """
...
class _CombinedRegistry(KeyBindingsBase):
    ...
    def get_bindings_starting_with_keys(self,
                                        keys: KeysTuple) -> list[Binding]:
...
def attach_winch_signal_handler(
    handler: Callable[[], None], ) -> Generator[None, None, None]:
    """
    ...
    loop = get_running_loop()
    previous_winch_handler = getattr(loop, "_signal_handlers",
                                     {}).get(sigwinch)
