...
class _CompiledGrammar:
    ...
    @classmethod
    def _transform(cls, root_node: Node,
                   create_group_func: Callable[[Variable], str]) -> str:
        """
    ...
    def _transform_prefix(
            cls, root_node: Node,
            create_group_func: Callable[[Variable], str]) -> Iterable[str]:
        """
        ...
        def transform(node: Node) -> Iterable[str]:
            ...
            if isinstance(node, AnyNode):
                ...
                if children_without_variable:
                    yield "|".join(r for c in children_without_variable
            ...
            # match right before the cursor.)
                ...
                if node.negative:
                    yield "(?!%s)" % cls._transform(node.childnode,
                                                    create_group_func)
                else:
    ...
    def match_prefix(self, string: str) -> Match | None:
        ...
        for patterns in [self._re_prefix, self._re_prefix_with_trailing_input]:
            ...
            if matches2 != []:
                return Match(string, matches2, self._group_names_to_nodes,
                             self.unescape_funcs)
...
class Match:
    ...
    def _nodes_to_values(self) -> list[tuple[str, str, tuple[int, int]]]:
        ...
        return [(varname, get(slice), slice)
    ...
    def variables(self) -> Variables:
        ...
        """
        return Variables([(k, self._unescape(k, v), sl)
...
class MatchVariable:
    ...
    def __init__(self, varname: str, value: str, slice: tuple[int,
                                                              int]) -> None:
