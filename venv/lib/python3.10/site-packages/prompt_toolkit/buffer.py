...
class Buffer:
    ...
    def __init__(
        ...
        self._document_cache: FastDictCache[tuple[str, int,
                                                  SelectionState | None],
                                            Document] = FastDictCache(Document,
                                                                      size=10)
    ...
    def document(self) -> Document:
        ...
        """
        return self._document_cache[self.text, self.cursor_position,
    ...
    def set_document(self,
                     value: Document,
                     bypass_readonly: bool = False) -> None:
        """
    ...
    def save_to_undo_stack(self, clear_redo_stack: bool = True) -> None:
        ...
        if self._undo_stack and self._undo_stack[-1][0] == self.text:
            self._undo_stack[-1] = (self._undo_stack[-1][0],
    ...
    def transform_region(self, from_: int, to: int,
        ...
        self.text = "".join([
            self.text[:from_] + transform_callback(self.text[from_:to]) +
            self.text[to:]
        ])
    ...
    def delete_before_cursor(self, count: int = 1) -> str:
        ...
        if self.cursor_position > 0:
            deleted = self.text[self.cursor_position -
                                count:self.cursor_position]

            new_text = (self.text[:self.cursor_position - count] +
    ...
    def _set_completions(self,
                         completions: list[Completion]) -> CompletionState:
        """
    ...
    def _history_matches(self, i: int) -> bool:
        ...
        return self.history_search_text is None or self._working_lines[
            i].startswith(self.history_search_text)
    ...
    def start_selection(
        ...
        """
        self.selection_state = SelectionState(self.cursor_position,
                                              selection_type)
    ...
    def paste_clipboard_data(
        ...
        assert paste_mode in (PasteMode.VI_BEFORE, PasteMode.VI_AFTER,
                              PasteMode.EMACS)
    ...
    def validate(self, set_cursor: bool = False) -> bool:
        ...
        if self.validator:
            try:
                ...
                if set_cursor:
                    self.cursor_position = min(max(0, e.cursor_position),
                                               len(self.text))
    ...
    def _search(
        ...
        def search_once(working_index: int,
                        document: Document) -> tuple[int, Document] | None:
            """
            Do search one time.
    ...
    def _create_completer_coroutine(
            self) -> Callable[..., Coroutine[Any, Any, None]]:
        """
        ...
        def completion_does_nothing(document: Document,
            ...
            replaced_text = text_before_cursor[len(text_before_cursor) +
            ...
            try:
                ...
                async with aclosing(
                        self.completer.get_completions_async(
                            document, complete_event)) as async_generator:
                        ...
                        if len(completions) > 1:
                            ...
                            completions[:] = [
                                c.new_completion_from_position(
                                    len(common_part)) for c in completions
                            ]
...
def reshape_text(buffer: Buffer, from_row: int, to_row: int) -> None:
    ...
    if lines_to_reformat:
        ...
        length = match.end(
        ) if match else 0  # `match` can't be None, actually.
