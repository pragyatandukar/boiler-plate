...
class TextObject:
    ...
    def operator_range(self, document: Document) -> tuple[int, int]:
        ...
        if (self.type == TextObjectType.EXCLUSIVE and
                doc.translate_index_to_position(end + doc.cursor_position)[1]
                == 0):
        ...
        if self.type == TextObjectType.LINEWISE:
            # Select whole lines
            row, col = doc.translate_index_to_position(start +
                                                       doc.cursor_position)
            start = doc.translate_row_col_to_index(row,
                                                   0) - doc.cursor_position
            row, col = doc.translate_index_to_position(end +
                                                       doc.cursor_position)
            end = (doc.translate_row_col_to_index(row, len(doc.lines[row])) -
                   doc.cursor_position)
        return start, end
    ...
    def cut(self, buffer: Buffer) -> tuple[Document, ClipboardData]:
        ...
        document = Document(
            buffer.text,
            to,
            SelectionState(original_cursor_position=from_,
...
def create_text_object_decorator(
    key_bindings: KeyBindings, ) -> Callable[..., Callable[[_TOF], _TOF]]:
    """
...
def create_operator_decorator(
    key_bindings: KeyBindings, ) -> Callable[..., Callable[[_OF], _OF]]:
    """
    Create a decorator that can be used for registering Vi operators.
    """

    def operator_decorator(*keys: Keys | str,
                           filter: Filter = Always(),
...
def load_vi_bindings() -> KeyBindingsBase:
    ...
    handle("enter",
           filter=is_returnable & ~is_multiline)(get_by_name("accept-line"))
    ...
    def in_block_selection() -> bool:
        ...
        return bool(buff.selection_state
    ...
    def create_delete_and_change_operators(delete_only: bool,
        ...
        def delete_or_change_operator(event: E,
                                      text_object: TextObject) -> None:
            clipboard_data = None
            buff = event.current_buffer
            ...
            if clipboard_data and clipboard_data.text:
                if with_register:
                    ...
                    if reg_name in vi_register_names:
                        event.app.vi_state.named_registers[
    ...
    def create_transform_handler(filter: Filter,
                                 transform_func: Callable[[str], str], *a:
                                 str) -> None:
    ...
    def create_ci_ca_handles(ci_start: str,
                             ci_end: str,
                             inner: bool,
    ...
    def _goto_first_line(event: E) -> TextObject:
        ...
        if event._arg:
            ...
            return TextObject(d.get_start_of_document_position(),
                              type=TextObjectType.LINEWISE)
    ...
    def _ge(event: E) -> TextObject:
        ...
        prev_end = event.current_buffer.document.find_previous_word_ending(
            count=event.arg)
        return TextObject(prev_end - 1 if prev_end is not None else 0,
                          type=TextObjectType.INCLUSIVE)
    ...
    def _last_line(event: E) -> TextObject:
        ...
        return TextObject(
            buf.document.translate_row_col_to_index(
                buf.document.line_count - 1, 0) - buf.cursor_position,
            type=TextObjectType.LINEWISE,
        )
    ...
    def _stop_macro(event: E) -> None:
        ...
        if vi_state.recording_register:
            vi_state.named_registers[
                vi_state.recording_register] = ClipboardData(
                    vi_state.current_recording)
