...
def _window_too_small() -> Window:
    ...
    return Window(
        FormattedTextControl(text=[("class:window-too-small",
                                    " Window too small... ")]))
...
class HSplit(_Split):
    ...
    def __init__(
        ...
        self._children_cache: SimpleCache[tuple[Container, ...],
                                          list[Container]] = SimpleCache(
                                              maxsize=1)
    ...
    def write_to_screen(
        ...
        if sizes is None:
            self.window_too_small.write_to_screen(screen, mouse_handlers,
                                                  write_position, style,
                                                  erase_bg, z_index)
        else:
            #
    ...
    def _divide_heights(self,
                        write_position: WritePosition) -> list[int] | None:
        """
...
class VSplit(_Split):
    ...
    def write_to_screen(
        ...
        height = max(write_position.height,
                     min(write_position.height, max(heights)))
...
class FloatContainer(Container):
    ...
    def write_to_screen(
        ...
        self.content.write_to_screen(screen, mouse_handlers, write_position,
    ...
    def _draw_float(
        ...
        cpos = screen.get_menu_position(fl.attach_to_window
                                        or get_app().layout.current_window)
        ...
        # Near x position of cursor.
            ...
            if fl_width is None:
                width = fl.content.preferred_width(
                    write_position.width).preferred
    ...
    def _area_is_empty(self, screen: Screen,
                       write_position: WritePosition) -> bool:
        """
...
class WindowRenderInfo:
    ...
    def displayed_lines(self) -> list[int]:
        ...
        """
        return sorted(row
                      for row, col in self.visible_line_to_row_col.values())
    ...
    def last_visible_line(self, before_scroll_offset: bool = False) -> int:
        ...
        if before_scroll_offset:
            return self.displayed_lines[-1 -
                                        self.applied_scroll_offsets.bottom]
        else:
            return self.displayed_lines[-1]
...
class ColorColumn:
    ...
    def __init__(self,
                 position: int,
                 style: str = "class:color-column") -> None:
...
class Window(Container):
    ...
    def __init__(
        ...
        self._ui_content_cache: SimpleCache[tuple[int, int, int],
                                            UIContent] = SimpleCache(maxsize=8)
        self._margin_width_cache: SimpleCache[tuple[Margin, int],
                                              int] = SimpleCache(maxsize=1)
    ...
    def write_to_screen(
        ...
        if self.dont_extend_height():
            write_position.height = min(
                write_position.height,
                self.preferred_height(write_position.width,
                                      write_position.height).preferred,
            )
    ...
    def _write_to_screen_at_index(
        ...
        # Scroll content.
        wrap_lines = self.wrap_lines()
        self._scroll(ui_content, write_position.width - total_margin_width,
                     write_position.height)
        ...
        def render_margin(m: Margin, width: int) -> UIContent:
            ...
            fragments = m.create_margin(render_info, width,
                                        write_position.height)
        ...
        for m, width in zip(self.left_margins, left_margin_widths):
            if width > 0:  # (ConditionalMargin returns a zero width. -- Don't render.)
                ...
                # Copy and shift X.
                self._copy_margin(margin_content, screen, write_position,
                                  move_x, width)
                move_x += width
        ...
        for m, width in zip(self.right_margins, right_margin_widths):
            ...
            # Copy and shift X.
            self._copy_margin(margin_content, screen, write_position, move_x,
                              width)
            move_x += width
    ...
    def _copy_body(
    ...
    ) -> tuple[dict[int, tuple[int, int]], dict[tuple[int, int], tuple[int,
                                                                       int]]]:
        """
                        ...
                        # Merge it in the previous cell.
                            ...
                            for pw in [2, 1]:  # Previous character width.
                                if (x - pw >= 0
                                    ...
                                    char2 = _CHAR_CACHE[prev_char.char + c,
                                                        prev_char.style]
        ...
        if ui_content.menu_position:
            new_screen.set_menu_position(
                self,
                cursor_pos_to_screen_pos(ui_content.menu_position.y,
                                         ui_content.menu_position.x),
            )

        # Update output screen height.
        new_screen.height = max(new_screen.height,
                                ypos + write_position.height)
    ...
    def _apply_style(self, new_screen: Screen, write_position: WritePosition,
    ...
    def _highlight_cursorlines(self, new_screen: Screen, cpos: Point, x: int,
        ...
        if self.cursorline():
            ...
            for x in range(x, x + width):
                original_char = row[x]
                row[x] = _CHAR_CACHE[original_char.char,
                                     original_char.style + cursor_line_style]
        ...
        if self.cursorcolumn():
            for y2 in range(y, y + height):
                ...
                row[cpos.x] = _CHAR_CACHE[original_char.char,
                                          original_char.style +
                                          cursor_column_style]
        ...
        for cc in colorcolumns:
            ...
            if column < x + width:  # Only draw when visible.
                ...
                for y2 in range(y, y + height):
                    ...
                    row[column + x] = _CHAR_CACHE[original_char.char,
                                                  original_char.style +
                                                  color_column_style]
    ...
    def _copy_margin(
        ...
        margin_write_position = WritePosition(xpos, ypos, width,
                                              write_position.height)
        self._copy_body(margin_content, new_screen, margin_write_position, 0,
                        width)
    ...
    def _scroll_when_linewrapping(self, ui_content: UIContent, width: int,
        ...
        def get_line_height(lineno: int) -> int:
            return ui_content.get_height_for_line(lineno, width,
                                                  self.get_line_prefix)
        ...
        if line_height > height - scroll_offsets_top:
            ...
            self.vertical_scroll_2 = max(0, text_before_height - height,
                                         self.vertical_scroll_2)
            return
        else:
            self.vertical_scroll_2 = 0
        ...
        self.vertical_scroll = min(self.vertical_scroll,
    ...
    def _scroll_without_linewrapping(self, ui_content: UIContent, width: int,
                                     height: int) -> None:
        """
        Scroll to make sure the cursor position is visible and that we maintain
        the requested scroll offset.
    ...
    def _scroll_up(self) -> None:
        ...
        if info.vertical_scroll > 0:
            ...
            if (info.cursor_position.y >= info.window_height - 1 -
...
class ConditionalContainer(Container):
    ...
    def write_to_screen(
        ...
        if self.filter():
            return self.content.write_to_screen(screen, mouse_handlers,
                                                write_position, parent_style,
                                                erase_bg, z_index)
...
class DynamicContainer(Container):
    ...
    def preferred_height(self, width: int,
                         max_available_height: int) -> Dimension:
        return self._get_container().preferred_height(width,
                                                      max_available_height)
    ...
    def write_to_screen(
        ...
        self._get_container().write_to_screen(screen, mouse_handlers,
                                              write_position, parent_style,
                                              erase_bg, z_index)
