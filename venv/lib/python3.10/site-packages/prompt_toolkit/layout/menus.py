...
class CompletionsMenuControl(UIControl):
    ...
    def _get_menu_width(self, max_width: int,
                        complete_state: CompletionState) -> int:
        """
    ...
    def _get_menu_meta_width(self, max_width: int,
                             complete_state: CompletionState) -> int:
        """
    ...
    def _get_menu_item_meta_fragments(self, completion: Completion,
...
def _get_menu_item_fragments(
    ...
    text, tw = _trim_formatted_text(completion.display,
...
class MultiColumnCompletionMenuControl(UIControl):
    ...
    def __init__(self,
                 min_rows: int = 3,
                 suggested_max_column_width: int = 30) -> None:
        assert min_rows >= 1
    ...
    def create_content(self, width: int, height: int) -> UIContent:
        ...
        def grouper(
                ...
                fillvalue: _T | None = None) -> Iterable[Sequence[_T | None]]:
    ...
    def _get_column_width(self, completion_state: CompletionState) -> int:
        ...
        try:
            ...
            result = (max(
                get_cwidth(c.display_text)
                for c in completion_state.completions) + 1)
    ...
    def mouse_handler(self, mouse_event: MouseEvent) -> NotImplementedOrNone:
        ...
        def scroll_left() -> None:
            b.complete_previous(count=self._rendered_rows,
                                disable_wrap_around=True)
            self.scroll = max(0, self.scroll - 1)

        def scroll_right() -> None:
            b.complete_next(count=self._rendered_rows,
                            disable_wrap_around=True)
