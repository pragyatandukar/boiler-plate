...
class Percentage(Formatter):
    ...
    def format(
        ...
        return HTML(
            self.template).format(percentage=round(progress.percentage, 1))
...
class Bar(Formatter):
    ...
    def format(
        ...
        return HTML(self.template).format(start=self.start,
                                          end=self.end,
                                          bar_a=bar_a,
                                          bar_b=bar_b,
                                          bar_c=bar_c)
...
class Progress(Formatter):
    ...
    def format(
        ...
        return HTML(self.template).format(current=progress.items_completed,
...
class TimeLeft(Formatter):
    ...
    def get_width(self, progress_bar: ProgressBar) -> AnyDimension:
        all_values = [
            ...
            if c.time_left is not None else 7 for c in progress_bar.counters
        ]
        if all_values:
            return max(all_values)
        return 0
