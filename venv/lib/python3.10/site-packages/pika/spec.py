...
class Connection(amqp_object.Class):
    ...
    class Secure(amqp_object.Method):
        ...
        def encode(self):
            ...
            value = self.challenge.encode('utf-8') if isinstance(
                self.challenge, unicode_type) else self.challenge
            pieces.append(struct.pack('>I', len(value)))
            pieces.append(value)
            return pieces
    ...
    class OpenOk(amqp_object.Method):
        ...
        def decode(self, encoded, offset=0):
            self.known_hosts, offset = data.decode_short_string(
                encoded, offset)
            return self
    ...
    class UpdateSecret(amqp_object.Method):
        ...
        def encode(self):
            ...
            value = self.new_secret.encode('utf-8') if isinstance(
                self.new_secret, unicode_type) else self.new_secret
            pieces.append(struct.pack('>I', len(value)))
            pieces.append(value)
            assert isinstance(self.reason, str_or_bytes),\
                'A non-string value was supplied for self.reason'
...
class Channel(amqp_object.Class):
    ...
    class Open(amqp_object.Method):
        ...
        def decode(self, encoded, offset=0):
            self.out_of_band, offset = data.decode_short_string(
                encoded, offset)
            return self
    ...
    class OpenOk(amqp_object.Method):
        ...
        def encode(self):
            ...
            value = self.channel_id.encode('utf-8') if isinstance(
                self.channel_id, unicode_type) else self.channel_id
            pieces.append(struct.pack('>I', len(value)))
            pieces.append(value)
            return pieces
...
class Exchange(amqp_object.Class):
    ...
    class Bind(amqp_object.Method):
        ...
        def decode(self, encoded, offset=0):
            self.ticket = struct.unpack_from('>H', encoded, offset)[0]
            offset += 2
            self.destination, offset = data.decode_short_string(
                encoded, offset)
            self.source, offset = data.decode_short_string(encoded, offset)
            self.routing_key, offset = data.decode_short_string(
                encoded, offset)
            bit_buffer = struct.unpack_from('B', encoded, offset)[0]
            offset += 1
