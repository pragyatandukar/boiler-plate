            ...
            for method in paths[route]:
                ...
                if operation_id in ids:
                    warnings.warn(
                        ...
                        .format(route1=ids[operation_id]['route'],
        ...
        for path, method, view in view_endpoints:
            ...
            for k in components.keys():
                ...
                if components_schemas[k] == components[k]:
                    continue
                warnings.warn(
                    'Schema component "{}" has been overridden with a different value.'
                    .format(k))
...
class AutoSchema(ViewInspector):
    ...
    def map_choicefield(self, field):
        choices = list(dict.fromkeys(
            field.choices))  # preserve order and remove duplicates
        if all(isinstance(choice, bool) for choice in choices):
            type = 'boolean'
        elif all(isinstance(choice, int) for choice in choices):
            type = 'integer'
        elif all(
                isinstance(choice, (int, float, Decimal))
                for choice in choices):  # `number` includes `integer`
