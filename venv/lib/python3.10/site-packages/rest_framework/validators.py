...
class UniqueValidator:
    ...
    def __repr__(self):
        return '<%s(queryset=%s)>' % (self.__class__.__name__,
...
class UniqueTogetherValidator:
    ...
    def __call__(self, attrs, serializer):
        ...
        if serializer.instance is None:
            ...
            checked_values = [
                value for field, value in attrs.items() if field in self.fields
                and value != getattr(serializer.instance, field)
            ]

        if checked_values and None not in checked_values and qs_exists(
                queryset):
    ...
    def __repr__(self):
        return '<%s(queryset=%s, fields=%s)>' % (self.__class__.__name__,
                                                 smart_repr(self.queryset),
...
class BaseUniqueForValidator:
    ...
    def __call__(self, attrs, serializer):
        ...
        queryset = self.queryset
        queryset = self.filter_queryset(attrs, queryset, field_name,
                                        date_field_name)
        queryset = self.exclude_current_instance(attrs, queryset,
                                                 serializer.instance)
        if qs_exists(queryset):
    ...
    def __repr__(self):
        return '<%s(queryset=%s, field=%s, date_field=%s)>' % (
            ...
            smart_repr(self.field), smart_repr(self.date_field))
