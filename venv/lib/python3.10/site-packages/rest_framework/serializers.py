...
LIST_SERIALIZER_KWARGS = ('read_only', 'write_only', 'required', 'default',
                          'initial', 'source', 'label', 'help_text', 'style',
                          'error_messages', 'allow_empty', 'instance', 'data',
                          'partial', 'context', 'allow_null', 'max_length',
                          'min_length')
...
class BaseSerializer(Field):
    ...
    def many_init(cls, *args, **kwargs):
        ...
        list_kwargs.update({
            key: value
            for key, value in kwargs.items() if key in LIST_SERIALIZER_KWARGS
        })
...
class SerializerMetaclass(type):
    ...
    def _get_declared_fields(cls, bases, attrs):
        ...
        base_fields = [(visit(name), f) for base in bases
...
class Serializer(BaseSerializer, metaclass=SerializerMetaclass):
    ...
    def _read_only_defaults(self):
        fields = [
            ...
            if (field.read_only) and (field.default != empty) and (
                field.source != '*') and ('.' not in field.source)
        ]
...
class ListSerializer(BaseSerializer):
    ...
    def to_representation(self, data):
        ...
        iterable = data.all() if isinstance(
            data, models.manager.BaseManager) else data
...
def raise_errors_on_nested_writes(method_name, serializer, validated_data):
    ...
    * Automatically create a profile instance.
    ...
    assert not any(
        isinstance(field, BaseSerializer) and (field.source in validated_data)
    ...
    assert not any(
        ...
        (field.source_attrs[0] in model_field_info.relations)
...
class ModelSerializer(Serializer):
    ...
    * Default `.create()` and `.update()` implementations are provided.
    ...
    def get_default_field_names(self, declared_fields, model_info):
        ...
        """
        return ([model_info.pk.name] + list(declared_fields) +
                list(model_info.fields) + list(model_info.forward_relations))
    ...
    def build_standard_field(self, field_name, model_field):
        ...
        if (postgres_fields and
                isinstance(model_field,
    ...
    def build_unknown_field(self, field_name, model_class):
        ...
        raise ImproperlyConfigured(
            ...
            (field_name, model_class.__name__, self.__class__.__module__,
            ...
            for constraint in parent_class._meta.constraints:
                if isinstance(constraint, models.UniqueConstraint) and len(
                    ...
                    yield (constraint.fields, model._default_manager
                           if constraint.condition is None else
                           model._default_manager.filter(constraint.condition))
    ...
    def get_uniqueness_extra_kwargs(self, field_names, declared_fields,
        ...
        model = getattr(self.Meta, 'model')
        model_fields = self._get_model_fields(field_names, declared_fields,
                                              extra_kwargs)
    ...
    def get_unique_together_validators(self):
        ...
        for unique_together, queryset in self.get_unique_together_constraints(
            ...
            validator = UniqueTogetherValidator(queryset=queryset,
                                                fields=field_names)
...
class HyperlinkedModelSerializer(ModelSerializer):
    ...
    * Relationships to other instances are hyperlinks, instead of primary keys.
    ...
    def get_default_field_names(self, declared_fields, model_info):
        ...
        `Meta.fields` option is not specified.
        """
        return ([self.url_field_name] + list(declared_fields) +
                list(model_info.fields) + list(model_info.forward_relations))
