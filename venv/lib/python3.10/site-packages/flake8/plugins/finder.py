...
def _flake8_plugins(
    ...
    for ep in eps:
        ...
        if ep.name == "F":
            ...
            yield Plugin(pycodestyle_meta["name"], pycodestyle_meta["version"],
                         ep)
        else:
...
def _find_local_plugins(
    cfg: configparser.RawConfigParser, ) -> Generator[Plugin, None, None]:
    for plugin_type in ("extension", "report"):
        group = f"flake8.{plugin_type}"
        for plugin_s in utils.parse_comma_separated_list(
                cfg.get("flake8:local-plugins", plugin_type,
                        fallback="").strip(),
...
def _check_required_plugins(
    ...
    plugin_names = {
        utils.normalize_pypi_name(plugin.package)
...
def _classify_plugins(
    ...
    for loaded in plugins:
        if (getattr(loaded.obj, "off_by_default", False) and
                loaded.plugin.entry_point.name not in opts.enable_extensions):
