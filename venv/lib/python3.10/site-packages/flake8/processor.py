...
class FileProcessor:
    ...
    def multiline_string(
            self, token: tokenize.TokenInfo) -> Generator[str, None, None]:
        ...
        for token_type, text, start, end, line in self.tokens:
            ...
            if previous_row:
                ...
                if previous_row != start_row:
                    ...
                    if previous_text == "," or (previous_text not in "{[("
                                                and text not in "}])"):
                        text = f" {text}"
...
def is_multiline_string(token: tokenize.TokenInfo) -> bool:
    ...
    return token.type == FSTRING_END or (token.type == tokenize.STRING
                                         and "\n" in token.string)
