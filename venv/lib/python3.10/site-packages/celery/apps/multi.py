...
class Node:
    ...
    def __init__(self,
        ...
        self.options = self._annotate_with_default_opts(options
                                                        or OrderedDict())
    ...
    def _annotate_with_default_opts(self, options):
        options['-n'] = self.name
        self._setdefaultopt(options, ['--pidfile', '-p'],
                            '/var/run/celery/%n.pid')
        self._setdefaultopt(options, ['--logfile', '-f'],
    ...
    def start(self, env=None, **kwargs):
        return self._waitexec(self.argv,
...
class MultiParser:
    ...
    def parse(self, p):
        ...
        return (self._node_from_options(p, name, prefix, suffix, cmd, append,
                                        options) for name in names)

    def _node_from_options(self, p, name, prefix, suffix, cmd, append,
                           options):
        namespace, nodename, _ = build_nodename(name, prefix, suffix)
        namespace = nodename if nodename in p.namespaces else namespace
        return Node(nodename, cmd, append, p.optmerge(namespace, options),
                    p.passthrough)
