...
class CursesMonitor:  # pragma: no cover
    ...
    def selection_info(self):
        ...
        def alert_callback(mx, my, xs):
            ...
            infoitems = [('args', info.pop('args', None)),
                         ('kwargs', info.pop('kwargs', None))] + list(
                             info.items())
    ...
    def display_task_row(self, lineno, task):
        ...
        line = self.format_row(task.uuid, task.name, hostname, timef,
                               task.state)
        ...
        with self.lock:
            ...
            win.addstr(
                ...
                STATUS_SCREEN.format(
                    s=self.state,
                    w_alive=len(
                        [w for w in self.state.workers.values() if w.alive]),
    ...
    def workers(self):
        return [
            hostname for hostname, w in self.state.workers.items() if w.alive
        ]
...
def capture_events(app, state, display):  # pragma: no cover
    ...
    def on_connection_error(exc, interval):
        print('Connection Error: {!r}.  Retry in {}s.'.format(exc, interval),
              file=sys.stderr)
