...
class TestWorkController(worker.WorkController):
    ...
    def start(self):
        if self.logger_queue:
            ...
            handler.addFilter(lambda r: r.process != self.pid and not getattr(
                r, 'from_queue', False))
    ...
    def on_consumer_ready(self, consumer):
        ...
        test_worker_started.send(sender=self.app,
                                 worker=self,
                                 consumer=consumer)
        ...
        with _start_worker_thread(app,
            ...
            if perform_ping_check:
                ...
                with allow_join_result():
                    assert ping.delay().get(
                        timeout=ping_task_timeout) == 'pong'
...
def setup_app_for_worker(app: Celery, loglevel: Union[str, int],
                         logfile: str) -> None:
