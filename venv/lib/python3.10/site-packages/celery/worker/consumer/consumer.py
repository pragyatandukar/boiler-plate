...
class Consumer:
    ...
    def _update_prefetch_count(self, index=0):
        ...
        self.initial_prefetch_count = (self.pool.num_processes *
                                       self.prefetch_multiplier)
        return self._update_qos_eventually(index)

    def _update_qos_eventually(self, index):
        return (self.qos.decrement_eventually
                if index < 0 else self.qos.increment_eventually)(
    ...
    def _get_connection_retry_type(self, is_connection_loss_on_startup):
        return ('broker_connection_retry_on_startup' if
                (is_connection_loss_on_startup and
                 self.app.conf.broker_connection_retry_on_startup is not None)
    ...
    def on_connection_error_after_connected(self, exc):
        ...
        self.initial_prefetch_count = max(
            self.prefetch_multiplier, self.max_prefetch_count -
    ...
    def loop_args(self):
        return (self, self.connection, self.task_consumer, self.blueprint,
                self.hub, self.qos, self.amqheartbeat, self.app.clock,
    ...
    def add_task_queue(self,
        ...
        if queue in queues:
            ...
            exchange_type = ('direct'
                             if exchange_type is None else exchange_type)
    ...
    def _restore_prefetch_count_after_connection_restart(self, p, *args):
        with self.qos._mutex:
            ...
            new_prefetch_count = min(self.max_prefetch_count,
