...
class Panel(UserDict):
    ...
    def _register(cls,
        ...
        def _inner(fun):
            ...
            cls.meta[control_name] = controller_info_t(alias, type, visible,
                                                       default_timeout, _help,
                                                       signature, args,
                                                       variadic)
            if alias:
                cls.data[alias] = fun
...
def revoke_by_stamped_headers(state,
                              headers,
                              terminate=False,
                              signal=None,
                              **kwargs):
    ...
    for header, stamps in headers.items():
        updated_stamps = maybe_list(
            worker_state.revoked_stamps.get(header) or []) + list(
                maybe_list(stamps))
            ...
            for expected_header_key, expected_header_value in headers.items():
                if expected_header_key in req.stamps:
                    ...
                    if matching_stamps_for_request:
                        terminated_scheme_to_stamps_mapping[
                            expected_header_key].update(
...
@control_command(variadic='task_id',
                 args=[('signal', str)],
...
def reserved(state, **kwargs):
    ...
    reserved_tasks = (state.tset(worker_state.reserved_requests) -
...
def registered(state, taskinfoitems=None, builtins=False, **kwargs):
    ...
    tasks = reg if builtins else (task for task in reg
...
def add_consumer(state,
    ...
    state.consumer.call_soon(state.consumer.add_task_queue, queue, exchange,
                             exchange_type or 'direct', routing_key, **options)
