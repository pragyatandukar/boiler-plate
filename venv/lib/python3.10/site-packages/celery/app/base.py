...
def app_has_custom(app, attr):
    ...
    """
    return mro_lookup(app.__class__,
                      attr,
                      stop={Celery, object},
...
class Celery:
    ...
    def _autodiscover_tasks_from_fixups(self, related_name):
        return self._autodiscover_tasks_from_names([
            pkg
            for fixup in self._fixups if hasattr(fixup, 'autodiscover_tasks')
    ...
    def send_task(self,
        ...
        if expires is not None:
            if isinstance(expires, datetime):
                expires_s = (maybe_make_aware(expires) -
                             self.now()).total_seconds()
            elif isinstance(expires, str):
                expires_s = (maybe_make_aware(isoparse(expires)) -
                             self.now()).total_seconds()
            else:
                expires_s = expires
    ...
    def _load_config(self):
        ...
        while pending_beat:
            ...
            self._add_periodic_task(*periodic_task_args,
