...
class AsyncResult(ResultBase):
    ...
    def revoke(self,
        ...
        Arguments:
            ...
            timeout (float): Time in seconds to wait for replies when
                ``wait`` is enabled.
        """
        self.app.control.revoke(self.id,
    ...
    def revoke_by_stamped_headers(self,
        ...
        Arguments:
            ...
            timeout (float): Time in seconds to wait for replies when
                ``wait`` is enabled.
        """
        self.app.control.revoke_by_stamped_headers(headers,
                                                   connection=connection,
                                                   terminate=terminate,
                                                   signal=signal,
                                                   reply=wait,
    ...
    def maybe_throw(self, propagate=True, callback=None):
        ...
        state, value, tb = (cache['status'], cache['result'],
                            cache.get('traceback'))
...
class ResultSet(ResultBase):
    ...
    def _iter_meta(self, **kwargs):
        return (meta for _, meta in self.backend.get_many(
            {r.id
             for r in self.results}, max_iterations=1, **kwargs))

    def _failed_join_report(self):
        return (res for res in self.results if res.backend.is_cached(res.id)
                and res.state in states.PROPAGATE_STATES)
...
class GroupResult(ResultSet):
    ...
    def as_tuple(self):
        return ((self.id, self.parent and self.parent.as_tuple()),
    ...
    def restore(cls, id, backend=None, app=None):
        ...
        app = app or (cls.app
                      if not isinstance(cls.app, property) else current_app)
