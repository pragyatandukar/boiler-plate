...
def first(predicate, it):
    ...
    """
    return next(
        (v for v in evaluate_promises(it)
    ...
    def _matcher(it, *args, **kwargs):
        for obj in it:
            try:
                ...
                reply = (on_call(meth, *args, **kwargs) if on_call else meth(
                    *args, **kwargs))
...
def _argsfromspec(spec, replace_defaults=True):
    if spec.defaults:
        ...
        defaults = (list(range(len(spec.defaults)))
                    if replace_defaults else spec.defaults)
    ...
    return ', '.join(
        filter(None, [
            ...
            '*' if
            (kwonlyargs or kwonlyargs_optional) and not varargs else None,
...
def fun_takes_argument(name, fun, position=None):
    ...
    return (spec.varkw or spec.varargs
...
def fun_accepts_kwargs(fun):
    ...
    return any(p for p in inspect.signature(fun).parameters.values()
