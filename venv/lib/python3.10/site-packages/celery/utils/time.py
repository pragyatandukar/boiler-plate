...
def humanize_seconds(secs: int,
    ...
    if microseconds and secs > 0.0:
        return '{prefix}{sep}{0:.2f} seconds'.format(secs,
                                                     sep=sep,
                                                     prefix=prefix)
    return now
...
def maybe_make_aware(dt: datetime,
                     tz: tzinfo | None = None,
...
def utcoffset(
        ...
        localtime: Callable[...,
                            _time.struct_time] = _time.localtime) -> float:
...
def get_exponential_backoff_interval(factor: int,
                                     retries: int,
                                     maximum: int,
                                     full_jitter: bool = False) -> int:
