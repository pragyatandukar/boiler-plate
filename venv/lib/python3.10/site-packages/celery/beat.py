...
class BeatLazyFunc:
    ...
    def delay(self):
        return self._func(*self._func_params["args"],
                          **self._func_params["kwargs"])
...
class Scheduler:
    ...
    def __init__(self,
        ...
        self.sync_every_tasks = (app.conf.beat_sync_every if sync_every_tasks
                                 is None else sync_every_tasks)
        if not lazy:
            self.setup_schedule()
    ...
    def _when(self, entry, next_time_to_run, mktime=timegm):
        ...
        return (mktime(as_now.utctimetuple()) + as_now.microsecond / 1e6 +
    ...
    def tick(self,
        ...
        return min(
            ...
            if is_numeric_value(adjusted_next_time_to_run) else max_interval,
            max_interval)
    ...
    def should_sync(self):
        return ((not self._last_sync or
                 (time.monotonic() - self._last_sync) > self.sync_every)
    ...
    def apply_async(self, entry, producer=None, advance=True, **kwargs):
        ...
        try:
            ...
            if task:
                return task.apply_async(entry_args,
                                        entry_kwargs,
                                        producer=producer,
                                        **entry.options)
            else:
