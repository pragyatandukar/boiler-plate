    ...
    def _get_or_create_table(self):
        ...
        try:
            ...
            if error_code == 'ResourceNotFoundException':
                ...
                logger.info('DynamoDB Table {} is now available.'.format(
                    self.table_name))
        ...
        if missing_methods:
            logger.error(
                ('boto3 method(s) {methods} not found; ensure that '
                 'boto3>=1.9.178 and botocore>=1.12.178 are installed').format(
                     methods=','.join(missing_methods)))
            raise AttributeError('boto3 method(s) {methods} not found'.format(
                methods=','.join(missing_methods)))
        ...
        if status in ('ENABLED', 'ENABLING'):
                ...
                if cur_attr_name == self._ttl_field.name:
                    ...
                    logger.debug(
                        ('DynamoDB Time to Live is {situation} '
                         'on table {table}').format(
                             situation='already enabled' if status == 'ENABLED'
            ...
            if not self._has_ttl():
                ...
                logger.debug(
                    ('DynamoDB Time to Live is {situation} '
                     'on table {table}').format(
                         situation='already disabled' if status == 'DISABLED'
        ...
        # a log showing the unknown state.
            ...
            logger.warning(
                ('Unknown DynamoDB Time to Live status {status} '
                 'on table {table}. Attempting to continue.').format(
        ...
        try:
            ...
            logger.info(
                ('DynamoDB table Time to Live updated: '
                 'table={table} enabled={enabled} attribute={attr}').format(
                     table=self.table_name,
                     enabled=self._has_ttl(),
    ...
    def _wait_for_table_status(self, expected='ACTIVE'):
        ...
        while not achieved_state:
            table_description = self.client.describe_table(
                TableName=self.table_name)
            logger.debug('Waiting for DynamoDB table {} to become {}.'.format(
