...
class ResultConsumer(BaseResultConsumer):
    ...
    def _reconnect_pubsub(self):
        ...
        if self.subscribed_to:
            ...
            self._pubsub.connection.register_connect_callback(
                self._pubsub.on_connect)
        ...
        if self._pubsub:
            with self.reconnect_on_error():
                ...
                if message and message['type'] == 'message':
                    self.on_state_change(self._decode_result(message['data']),
                                         message)
        elif timeout:
            time.sleep(timeout)
...
class RedisBackend(BaseKeyValueStoreBackend, AsyncBackendMixin):
    ...
    def ensure(self, fun, args, **policy):
        ...
        return retry_over_time(fun, self.connection_errors, args, {},
    ...
    def apply_chord(self, header_result_args, body, **kwargs):
        ...
        if not isinstance(header_result_args[1], _regen):
            ...
            if any(
                    isinstance(nr, GroupResult)
                    for nr in header_result.results):
...
class SentinelBackend(RedisBackend):
    ...
    def _get_sentinel_instance(self, **params):
        ...
        min_other_sentinels = self._transport_options.get(
