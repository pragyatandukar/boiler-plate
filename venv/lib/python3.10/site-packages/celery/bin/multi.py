...
class MultiTool(TermLogger):
    ...
    def show(self, cluster):
        return self.ok('\n'.join(' '.join(node.argv_with_executable)
                                 for node in cluster))
    ...
    def expand(self, template, *argv):
        return self.ok('\n'.join(
            node.expander(template) for node in self.cluster_from_argv(argv)))
    ...
    def on_node_restart(self, node):
        self.note(self.colored.blue(f'> Restarting node {node.name}: '),
                  newline=False)
    ...
    def on_node_signal(self, node, sig):
        self.note('Sending {sig} to node {0.name} ({0.pid})'.format(node,
                                                                    sig=sig))
