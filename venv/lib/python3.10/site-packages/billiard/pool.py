...
class MaybeEncodingError(Exception):
    ...
    def __str__(self):
        return "Error sending result: '%r'. Reason: '%r'." % (self.value,
                                                              self.exc)
                ...
                if req:
                    ...
                    if max_memory_per_child > 0:
                        ...
                        if used_kb > 0 and used_kb > max_memory_per_child:
                            warning(
                                MAXMEM_USED_FMT.format(used_kb,
...
class TimeoutHandler(PoolThread):
    ...
    def _process_by_pid(self, pid):
        return next(
            ((proc, i)
             for i, proc in enumerate(self.processes) if proc.pid == pid),
            (None, None))
            ...
            try:
                ...
                if not time_terminate:
                    ...
                    if now - time_terminate > 5.0:
                        ...
                        break
                    debug(
                        'result handler: all workers terminated, '
                        'timeout in %ss',
...
class Pool:
    ...
    def __init__(self,
        ...
        self.enable_timeouts = bool(enable_timeouts or self.timeout is not None
                                    or self.soft_timeout is not None)
    ...
    def create_result_handler(self, **extra_kwargs):
        return self.ResultHandler(self._outqueue,
                                  self._quick_get,
                                  self._cache,
                                  self._poll_result,
    ...
    def _process_by_pid(self, pid):
        return next(((proc, i)
                     for i, proc in enumerate(self._pool) if proc.pid == pid),
                    (None, None))
    ...
    def _join_exited_workers(self, shutdown=False):
        ...
        # WorkerLostError.
        for job in [
                job for job in list(self._cache.values())
            ...
            for job in list(self._cache.values()):
                acked_by_gone = next(
                    (pid for pid in job.worker_pids()
                     if pid in cleaned or pid not in all_pids), None)
    ...
    def _terminate_pool(cls, taskqueue, inqueue, outqueue, pool,
                        ...
                        timeout_handler, help_stuff_finish_args):
...
class ApplyResult:
    ...
    def _set(self, i, obj):
        with self._mutex:
            ...
            if (self._value is not None and self._error_callback
            ...
            if self._accept_callback:
                try:
                    ...
                    if self._send_ack and synqW_fd:
                        return self._send_ack(response, pid, self._job,
                                              synqW_fd)
            if self._send_ack and synqW_fd:
