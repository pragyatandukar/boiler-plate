...
class ClickCompleter(Completer):
    ...
    def _get_completion_from_autocompletion_functions(
        ...
        if HAS_CLICK_V8:
            autocompletions = param.shell_complete(autocomplete_ctx,
                                                   incomplete)
        else:
        ...
        for autocomplete in autocompletions:
            if isinstance(autocomplete, tuple):
                ...
                param_choices.append(
                    Completion(text_type(autocomplete.value),
                               -len(incomplete)))
        ...
        if not getattr(param.type, "case_sensitive", True):
            ...
            return [
                Completion(
                    ...
                    -len(incomplete),
                    display=text_type(
                        repr(choice) if " " in choice else choice),
                )
    ...
    def _get_completion_for_Boolean_type(self, param, incomplete):
        return [
            Completion(text_type(k),
                       -len(incomplete),
                       display_meta=text_type("/".join(v))) for k, v in {
                           "true": ("1", "true", "t", "yes", "y", "on"),
                           "false": ("0", "false", "f", "no", "n", "off"),
                       }.items() if any(i.startswith(incomplete) for i in v)
        ]

    def _get_completion_from_params(self, autocomplete_ctx, args, param,
                                    incomplete):
        ...
        if not HAS_CLICK_V8 and isinstance(param_type, click.Choice):
            choices.extend(
                self._get_completion_from_choices_click_le_7(
                    param, incomplete))
    ...
    def _get_completion_for_cmd_args(
        ...
        for param in ctx_command.params:
            if isinstance(param.type, click.types.UnprocessedParamType):
                ...
                choices.extend(
                    self._get_completion_from_params(autocomplete_ctx, args,
                                                     param, incomplete))
    ...
    def get_completions(self, document, complete_event=None):
        ...
        try:
            choices.extend(
                self._get_completion_for_cmd_args(self.ctx_command, incomplete,
                                                  self.parsed_ctx, args))

            if isinstance(self.ctx_command, click.MultiCommand):
                incomplete_lower = incomplete.lower()

                for name in self.ctx_command.list_commands(self.parsed_ctx):
                    command = self.ctx_command.get_command(
                        self.parsed_ctx, name)
