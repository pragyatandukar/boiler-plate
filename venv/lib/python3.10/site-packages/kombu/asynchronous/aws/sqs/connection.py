...
class AsyncSQSConnection(AsyncAWSQueryConnection):
    ...
    def make_request(self,
        ...
        if protocol == 'query':
            request = self._create_query_request(operation_name, params,
                                                 queue_url, verb)
        elif protocol == 'json':
            request = self._create_json_request(operation_name, params,
                                                queue_url)
        else:
    ...
    def delete_queue(self, queue, force_deletion=False, callback=None):
        return self.get_status('DeleteQueue',
                               None,
                               queue.id,
                               callback=callback)
