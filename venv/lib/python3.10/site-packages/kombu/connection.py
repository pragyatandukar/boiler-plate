...
class Connection:
    ...
    def switch(self, conn_str):
        ...
        self._closed = False
        conn_params = (
            parse_url(conn_str) if "://" in conn_str else {
                "hostname": conn_str
            }  # noqa
        )
    ...
    def _debug(self, msg, *args, **kwargs):
        if self._logger:  # pragma: no cover
            ...
            logger.debug(fmt.format(id=id(self), msg=str(msg)), *args,
                         **kwargs)

    def connect(self):
        """Establish connection to server immediately."""
        return self._ensure_connection(max_retries=1,
    ...
    def _ensure_connection(self,
        ...
        with ctx():
            return retry_over_time(self._connection_factory,
    ...
    @contextmanager
    def _reraise_as_library_errors(self,
    ...
    def _info(self, resolve=True):
        transport_cls = self.transport_cls
        if resolve:
            transport_cls = self.resolve_aliases.get(transport_cls,
                                                     transport_cls)
        D = self.transport.default_connection_params

        if not self.hostname and D.get('hostname'):
            logger.warning("No hostname was supplied. "
    ...
    def as_uri(
        ...
        mask='**',
        getfields=itemgetter('port', 'userid', 'password', 'virtual_host',
                             'transport')
    ) -> str:
        ...
        if self.transport.can_parse_url:
            ...
            try:
                return self.transport.as_uri(connection_as_uri,
                                             include_password, mask)
    ...
    def SimpleQueue(self,
        ...
        return SimpleQueue(channel or self, name, no_ack, queue_opts,
                           queue_args, exchange_opts, **kwargs)

    def SimpleBuffer(self,
                     name,
                     no_ack=None,
                     queue_opts=None,
                     queue_args=None,
