...
_MaybeChannelBoundType = TypeVar("_MaybeChannelBoundType",
                                 bound="MaybeChannelBound")


def unpickle_dict(cls: type[_ObjectType], kwargs: dict[str,
                                                       Any]) -> _ObjectType:
    return cls(**kwargs)
...
class Object:
    ...
    def as_dict(self, recurse: bool = False) -> dict[str, Any]:
        ...
        return {
            attr: f(getattr(self, attr), type)
            for attr, type in self.attrs
        }

    def __reduce__(
        self: _ObjectType
    ) -> tuple[Callable[[type[_ObjectType], dict[str, Any]], _ObjectType],
...
class MaybeChannelBound(Object):
    ...
    def _repr_entity(self, item: str = '') -> str:
        ...
        if self.is_bound:
            return '<{} bound to chan:{}>'.format(item or type(self).__name__,
