...
__all__ = ('Broadcast', 'maybe_declare', 'uuid', 'itermessages', 'send_reply',
           'collect_replies', 'insured', 'drain_consumer', 'eventloop')
...
def generate_oid(node_id, process_id, thread_id, instance):
    ent = '{:x}-{:x}-{:x}-{:x}'.format(node_id, process_id, thread_id,
                                       id(instance))
    try:
...
class Broadcast(Queue):
    ...
    def __init__(self,
        ...
        super().__init__(alias=alias or name,
                         queue=queue,
                         name=queue,
...
def collect_replies(conn, channel, queue, *args, **kwargs):
    ...
    received = False
    try:
        for body, message in itermessages(conn, channel, queue, *args,
                                          **kwargs):
            if not no_ack:
                message.ack()
            received = True
