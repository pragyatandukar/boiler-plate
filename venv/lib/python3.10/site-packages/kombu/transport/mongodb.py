...
class Channel(virtual.Channel):
    ...
    def _new_queue(self, queue, **kwargs):
        if self.ttl:
            self.queues.update_one({'_id': queue}, {
                '$set': {
                    '_id': queue,
                    'options': kwargs,
    ...
    def _put(self, queue, message, **kwargs):
        ...
        if self.ttl:
            ...
            if msg_expire is not None and (data['expire_at'] is None
                                           or msg_expire < data['expire_at']):
    ...
    def _queue_bind(self, exchange, routing_key, pattern, queue):
        if self.typeof(exchange).type == 'fanout':
            self._create_broadcast_cursor(exchange, routing_key, pattern,
                                          queue)
    ...
    def _parse_uri(self, scheme='mongodb://'):
        ...
        options = {
            ...
            'ssl':
            self.ssl,
            'connectTimeoutMS': (int(self.connect_timeout *
                                     1000) if self.connect_timeout else None),
        }
    ...
    def _ensure_indexes(self, database):
        ...
        if self.ttl:
            ...
            database[self.queues_collection].create_index([('expire_at', 1)],
    ...
    def _update_queues_expire(self, queue):
        ...
        self.routing.update_many({'queue': queue},
                                 {'$set': {
                                     'expire_at': expire_at
                                 }})
        self.queues.update_many({'_id': queue},
                                {'$set': {
                                    'expire_at': expire_at
                                }})
...
class Transport(virtual.Transport):
    ...
    channel_errors = (virtual.Transport.channel_errors +
                      (errors.ConnectionFailure, errors.OperationFailure))
    driver_type = 'mongodb'
    driver_name = 'pymongo'

    implements = virtual.Transport.implements.extend(exchange_type=frozenset(
        ['direct', 'topic', 'fanout']), )
