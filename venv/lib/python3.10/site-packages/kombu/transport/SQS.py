...
class QoS(virtual.QoS):
    ...
    def reject(self, delivery_tag, requeue=False):
        ...
        if routing_key and message and backoff_tasks and backoff_policy:
            self.apply_backoff_policy(routing_key, delivery_tag,
    ...
    def apply_backoff_policy(self, routing_key, delivery_tag, backoff_policy,
                             backoff_tasks):
        ...
        if task_name in backoff_tasks and policy_value is not None:
            ...
            c.change_message_visibility(QueueUrl=queue_url,
...
class Channel(virtual.Channel):
            ...
            if self.predefined_queues:
                raise UndefinedQueueException(
                    ("Queue with name '{}' must be "
                     "defined in 'predefined_queues'.").format(sqs_qname))
    ...
    def _get_async(self, queue, count=1, callback=None):
        ...
        return self._get_from_sqs(
            ...
            callback=transform(self._on_messages_ready, callback, q_url,
                               queue),
        )
    ...
    @cached_property
    def region(self):
        return (self.transport_options.get('region')
                or boto3.Session().region_name or self.default_region)
