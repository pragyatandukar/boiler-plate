...
def get_redis_error_classes():
    ...
    return error_classes_t(
        (virtual.Transport.connection_errors +
         (InconsistencyError, socket.error, IOError, OSError,
          exceptions.ConnectionError, exceptions.BusyLoadingError,
          exceptions.AuthenticationError, exceptions.TimeoutError)),
...
class MultiChannelPoller:
    ...
    def _client_registered(self, channel, client, cmd):
        ...
        return (client.connection._sock is not None
...
class Channel(virtual.Channel):
    ...
    from_transport_options = (
        ...
        ('sep', 'ack_emulation', 'unacked_key', 'unacked_index_key',
         'unacked_mutex_key', 'unacked_mutex_expire', 'visibility_timeout',
         'unacked_restore_limit', 'fanout_prefix', 'fanout_patterns',
         'global_keyprefix', 'socket_timeout', 'socket_connect_timeout',
         'socket_keepalive', 'socket_keepalive_options',
         'queue_order_strategy', 'max_connections', 'health_check_interval',
         'retry_on_timeout', 'priority_steps')  # <-- do not add comma here!
    )
    ...
    def _brpop_start(self, timeout=1):
        ...
        if not queues:
            return
        keys = [
            self._q_for_pri(queue, pri) for pri in self.priority_steps
            for queue in queues
        ] + [timeout or 0]
    ...
    def _filter_tcp_connparams(self,
                               socket_keepalive=None,
                               socket_keepalive_options=None,
                               **params):
        return params
...
if sentinel:
    ...
    class SentinelManagedSSLConnection(sentinel.SentinelManagedConnection,
                                       redis.SSLConnection):
...
class SentinelChannel(Channel):
    ...
    from_transport_options = Channel.from_transport_options + (
        'master_name', 'min_other_sentinels', 'sentinel_kwargs')
