...
class Channel(virtual.Channel):
    ...
    def _new_queue(self, queue, **_):
        ...
        with self._queue_lock(queue):
            try:
                return self.client.write(key=self._key_prefix(queue),
                                         dir=True,
                                         value=None)
    ...
    def _put(self, queue, payload, **_):
        ...
        with self._queue_lock(queue):
            ...
            if not self.client.write(
                    key=key, value=dumps(payload), append=True):
    ...
    def _get(self, queue, timeout=None):
        ...
        with self._queue_lock(queue):
            ...
            try:
                result = self.client.read(key=key,
                                          recursive=True,
                                          index=self.index,
    ...
    def _size(self, queue):
        ...
        with self._queue_lock(queue):
            ...
            logger.debug('Found %s keys under %s with index %s', size, key,
                         self.index)
            return size
