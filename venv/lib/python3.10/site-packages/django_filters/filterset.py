...
class FilterSetMetaclass(type):
    ...
    def get_declared_filters(cls, bases, attrs):
        ...
        base_filters = [(visit(name), f) for base in bases
...
class BaseFilterSet:
    ...
    def get_form_class(self):
        ...
        """
        fields = OrderedDict([(name, filter_.field)
    ...
    def get_fields(cls):
        ...
        if not isinstance(fields, dict):
            fields = [(f, [settings.DEFAULT_LOOKUP_EXPR]) for f in fields
                      if f not in exclude]
        else:
            fields = [(f, lookups) for f, lookups in fields.items()
                      if f not in exclude]
...
def filterset_factory(model, filterset=FilterSet, fields=None):
    ...
    return type(filterset)(str("%sFilterSet" % model._meta.object_name),
                           (filterset, ), {
                               "Meta": Meta
                           })
