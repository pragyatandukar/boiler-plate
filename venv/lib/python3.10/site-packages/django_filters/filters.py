    ...
    def label():
        ...
        def fget(self):
            if self._label is None and hasattr(self, "model"):
                self._label = label_for_filter(self.model, self.field_name,
                                               self.lookup_expr, self.exclude)
            return self._label
...
class DateRangeFilter(ChoiceFilter):
    ...
    def __init__(self, choices=None, filters=None, *args, **kwargs):
        ...
        all_choices = list(
            chain.from_iterable(
                [subchoice[0] for subchoice in choice[1]] if isinstance(
                    choice[1], (list, tuple))  # This is an optgroup
                else [choice[0]] for choice in self.choices))
    ...
    * ``field_class`` is an optional argument that allows you to set the inner
    ...
    def __init__(self,
        ...
        self.empty_label = kwargs.pop("empty_label",
                                      settings.EMPTY_CHOICE_LABEL)

        super(LookupChoiceFilter, self).__init__(field_name=field_name,
                                                 **kwargs)
...
class OrderingFilter(BaseCSVFilter, ChoiceFilter):
    ...
    * ``field_labels`` is an optional argument that allows you to customize
    ...
    def filter(self, qs, value):
        ...
        ordering = [
            self.get_ordering_value(param) for param in value
    ...
    def normalize_fields(cls, fields):
        ...
        assert all(
            isinstance(field, str) or isinstance(field, Iterable)
            and len(field) == 2  # may need to be wrapped in parens
            for field in fields
        ), "'fields' must contain strings or (field name, param name) pairs."

        return OrderedDict([(f, f) if isinstance(f, str) else f
                            for f in fields])

    def build_choices(self, fields, labels):
        ascending = [(param, labels.get(field, _(pretty_name(param))))
